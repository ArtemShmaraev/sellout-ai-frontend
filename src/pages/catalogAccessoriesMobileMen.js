...
const CatalogAccessoriesMobileMen = () => {
    const linksBrands = [
                        ...
                        {Array.from({length: 57}).map((_, idx) => (
                            <div key={idx} className={styles.brandCircle}>
                                <div className={styles.circle}>
                                    <Image
                                        src={`/Images New Frontend/Mobile/Men/Products/Accessories/Brands/${idx + 1}.png?v=${Date.now()}`}
                                        alt="Brand Image"
                    ...
                    <div className={styles.categoriesGrid}>
                        {['Кепки', 'Очки', 'Панамы', 'Часы', 'Оправы', 'Collectibles'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem}>
                                <Image
                                    src={`/Images New Frontend/Mobile/Men/Products/Accessories/Categories/${idx + 1}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
                    ...
                    <div className={styles.categoriesGrid4}>
                        {['Ремни', 'Шапки', 'Шарфы', 'Перчатки', 'Очешники', 'Духи', 'Косметика', 'Галстуки', 'Брелоки', 'Кольца', 'Цепочки', 'Браслеты', 'Мячи', 'Мячи', 'Мячи', 'Чехлы', 'Для техники', 'Другие'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem4}>
                                <Image
                                    src={`/Images New Frontend/Mobile/Men/Products/Accessories/Categories/${idx + 7}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
...
export default observer(CatalogAccessoriesMobileMen);