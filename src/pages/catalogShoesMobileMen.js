                        ...
                        {Array.from({length: 28}).map((_, idx) => (
                            <div key={idx} className={styles.brandCircle}>
                                <div className={styles.circle}>
                                    <Image
                                        src={`/Images New Frontend/Mobile/Men/Products/Shoes/Brands/${idx + 1}.png?v=${Date.now()}`}
                                        alt="Brand Image"
                    ...
                    <div className={styles.categoriesGrid}>
                        {['Кроссовки', 'Лоферы', 'Шлёпки'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem}>
                                <Image
                                    src={`/Images New Frontend/Mobile/Men/Products/Shoes/Categories/${idx + 1}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
                    ...
                    <div className={styles.categoriesGrid4}>
                        {['Слипоны', 'Мокасины', 'Эспадрильи', 'Зимние кросс..', 'Туфли', 'Мюли', 'Сандалии', 'Пляжные санд..', 'Все ботинки', 'Мартинсы', 'Тимберленды', 'Челси'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem4}>
                                <Image
                                    src={`/Images New Frontend/Mobile/Men/Products/Shoes/Categories/${idx + 4}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
...
export default observer(CatalogShoesDesktopMen);