                    ...
                    <div className={styles.categoriesGrid}>
                        {['Кепки', 'Очки', 'Панамы', 'Часы', 'Оправы', 'Collectibles', 'Ремни', 'Шапки', 'Шарфы', 'Кольца', 'Цепочки', 'Браслеты', 'Перчатки', 'Очешники', 'Духи', 'Косметика', 'Галстуки', 'Брелоки', 'Серьги', 'Чехлы', 'Мячи', 'Мячи', 'Для техники', 'Другие'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem}>
                                <Image
                                    src={`/Images New Frontend/Desktop/Women/Products/Accessories/Categories/${idx + 1}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
                        ...
                        {Array.from({length: 57}).map((_, idx) => (
                            <div key={idx} className={styles.brandCircle}>
                                <div className={styles.circle}>
                                    <Image
                                        src={`/Images New Frontend/Desktop/Women/Products/Accessories/Brands/${idx + 1}.png?v=${Date.now()}`}
                                        alt="Brand Image"
