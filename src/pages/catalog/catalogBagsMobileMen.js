                        ...
                        {Array.from({length: 54}).map((_, idx) => (
                            <div key={idx} className={styles.brandCircle}>
                                <div className={styles.circle}>
                                    <Image
                                        src={`/Images New Frontend/Mobile/Men/Products/Bags/Brands/${idx + 1}.png?v=${Date.now()}`}
                                        alt="Brand Image"
                    ...
                    <div className={styles.categoriesGrid}>
                        {['Все сумки', 'Сумки на пояс', 'Рюкзаки'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem}>
                                <Image
                                    src={`/Images New Frontend/Mobile/Men/Products/Bags/Categories/${idx + 1}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
                    ...
                    <div className={styles.categoriesGrid4}>
                        {['Спортивные', 'Кошельки', 'Клатчи', 'Кардхолдеры', 'Косметички', 'Портфели', 'Чемоданы', 'Обложки'].map((category, idx) => (
                            <div key={idx} className={styles.categoryItem4}>
                                <Image
                                    src={`/Images New Frontend/Mobile/Men/Products/Bags/Categories/${idx + 4}.png?v=${Date.now()}`}
                                    alt={category}
                                    width={700}
                                    height={700}
...
export default observer(CatalogBagsDesktopMen);